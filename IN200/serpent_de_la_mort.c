...
struct serpent { 
	...
	struct elem A[1001]; // les éléments constitutifs du serpent 
	int dirx,diry;
};
...
struct serpent deplace_serpent(struct serpent S){
	...
	if(S.diry == 1)  {S.A[S.longueur-1].j = modulo(S.A[S.longueur-1].j+1,SIZE_Y);}
	if(S.dirx == 1)  {S.A[S.longueur-1].i = modulo(S.A[S.longueur-1].i+1,SIZE_X);}
	if(S.diry == -1) {S.A[S.longueur-1].j = modulo(S.A[S.longueur-1].j-1,SIZE_Y);}
	if(S.dirx == -1) {S.A[S.longueur-1].i = modulo(S.A[S.longueur-1].i-1,SIZE_X);}
...
int detect_mange(struct serpent *S, struct elem J[SIZE_X][SIZE_Y]){
	if(S->longueur < 1000 && J[S->A[S->longueur-1].i + S->dirx][S->A[S->longueur-1].j + S->diry].forme != VIDE){
...
int main() {
	...
	while(game_ok(S,J)){
		...
		affiche_all();
		fill_screen(black);
		p.x = pow(-1,alea_int(2))*alea_int(2);
		p.y = pow(-1,alea_int(2))*alea_int(2);
		if(p.x != 0 || p.y != 0){
			S = change_dir(S,p);
		}
