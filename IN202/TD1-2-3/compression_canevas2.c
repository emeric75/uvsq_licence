...
void det_commun(texte t, int pos, int tab[]){
	int k = ((TAILLE_DICT > pos) ? 0 : pos-TAILLE_DICT);
	int ok; 
	int count = 0;
	for(int i = pos-1; i >= k; i--){
		ok = 1;
		for(int j = 0; j<DELTA_MIN; j++){
			if(pos+j >= t.taille || t.tableau[pos+j].lettre != t.tableau[i+j].lettre){ ok = 0; break;}
		}
		if(ok) {tab[count] = i; count++;}
	} 
	tab[count] = -1;
}
...
code max_sequence(texte t, int pos){
  ...
  det_commun(t, pos, tab); int k=0, maxi = 0;
  code res; res.longueur = 0; res.decalage = t.tableau[pos].lettre;
  while(tab[k] != -1){
	  int j = DELTA_MIN;
	  while(pos+j < t.taille && (t.tableau[pos+j].lettre == t.tableau[tab[k]+j].lettre)) j++;
	  if(j>maxi){res.longueur = j; res.decalage = pos - tab[k];}
	  k++;
  }
  return res;
}
